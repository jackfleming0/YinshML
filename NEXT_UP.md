...
1. **Heuristic-weight curriculum (1.0 → 0.3 → 0.0).** *Directly evidenced by the warm-start rerun.* Iter 1-2 regress because the value head is readjusting from supervised distribution to heuristic-mixed self-play distribution. Curriculum annealing is exactly what dampens that handoff. Cheap trainer/supervisor change.
2. **Tournament CI / SE reporting + opponent pool expansion.** `arena.games_per_match: 50` (not 200 as I'd assumed). At 50 games with p=0.5, 95% CI half-width is ~14% — so iter 9's 49.5% rejection has a true-rate CI of roughly [35%, 64%], wholly straddling the 55% gate. Promote/reject calls are being made on signal statistically indistinguishable from the threshold. Until this is fixed, every other Track A item's measured impact is noise. Log Wilson 95% CI per matchup, expand opponent pool to rolling top-K, and consider raising `games_per_match` (or, equivalently, lowering `tournament_sliding_window` to free games for the comparisons that matter).
3. **EMA checkpoint for tournament eval.** The yes-no-no-no-yes-no-no-no oscillation around iter 6 *might* mean latest-checkpoint-as-eval-target is too noisy — but with the CI math above, half of those rejections may be false negatives, not real regressions. Ship after #2 so the EMA-vs-no-EMA comparison has signal to measure against. Independent and low-risk.
4. **Deterministic eval mode.** No fixed-seed path in `_play_match`. Together with #2, this is the trustworthy-comparisons foundation that everything downstream depends on.
5. **Subtree reuse across moves.** Still the biggest single ELO lever in the menu, but it's the hardest engineering lift in Tier 1 — pull it after the cheap evidence-backed items land so the trustworthy-comparisons infrastructure is there to measure its impact.
6. **FPU in PUCT.** Tiny patch; near-zero cost.
7. **Soft value targets (Gaussian around class).** Small trainer change; hits the discrimination ceiling.
8. **Cosine + warmup LR schedule.**
9. **Promote `epsilon_mix` to config + taper.**
10. **Mixed-precision on MPS.** Less urgent now that the OOM is resolved; still a 20-30% wall-clock win.
11. **Value-head calibration + entropy logging.** Needed to *see* §5 probe results below.
12. **Rework move encoder to a collision-free scheme.** Audit 2026-04-14 found `((src*31 + dst) % 5848)` for 85-position src/dst produces only 2687 distinct values across 7140 (src,dst) pairs — 62% collision rate, 3161 slots structurally unreachable (output bounded to [0, 2688]). Multiple distinct ring moves share policy indices in MCTS training targets; the network can't learn to distinguish them. Candidate clean encoding: direct `(src_idx, dst_idx)` pair indexing (85 × 84 = 7140 slots, no collisions, fits inside the current 7395 budget). Invasive — touches `move_to_index`, `index_to_move`, and any consumer that assumes current slot layout. Deferred until after Track A items #1-#4 land so we're not stacking encoder changes on top of statistics fixes.
