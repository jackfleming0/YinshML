...
Order updated 2026-04-14 after the 10-iter warm-start results came in (and revised again the same day after reading `configs/training.yaml:59` confirmed `games_per_match: 50`, not 200). The iter 1-2 regression (-57, -24 ELO) directly evidences the curriculum item; the apparent iter 6 → iter 9 fall-back evidences EMA — but only if the rejections are statistically real, which at 50 games they may not be. Tournament CI/SE pulled to #3 because it's a prerequisite for knowing whether any other Track A item helped.

1. **Fix `_transform_state` + `_transform_coord` geometry (bounded by the D6 augmentation work).** *Promoted 2026-04-14 from implicit assumption.* D6 augmentation landed 2026-04-14 with the policy permutation correct, but two geometry bugs underneath it remain. (a) `_transform_coord` treats the YINSH board as D6-symmetric; actually only 180° preserves all 85 cells — the other 5 rotations preserve 39/85, reflections 29-57/85. Under those transforms, pieces on non-symmetric cells vanish from augmented states. (b) `_transform_state` copies the phase channel through the same partial coord_map, shrinking `np.mean(ch5)` so `decode_state` reports the wrong phase after rotation. Together these mean ≤4× of the claimed 12× data multiplier is geometrically faithful today. Two options: (i) fix the hex-rotation math to match YINSH's actual coordinate system (if it is D6-symmetric after all — verify first), or (ii) drop to `max_augmentations: 2` (identity + 180°) until the math is fixed. Until this lands, every other Track A item's measured impact bakes in noise from 11 malformed augmentations per sample.
...
3. **Tournament CI / SE reporting + opponent pool expansion.** *Promoted again 2026-04-14 from #4 → #3.* `arena.games_per_match: 50` (not 200 as I'd assumed). At 50 games with p=0.5, 95% CI half-width is ~14% — so iter 9's 49.5% rejection has a true-rate CI of roughly [35%, 64%], wholly straddling the 55% gate. Promote/reject calls are being made on signal statistically indistinguishable from the threshold. Until this is fixed, every other Track A item's measured impact is noise. Log Wilson 95% CI per matchup, expand opponent pool to rolling top-K, and consider raising `games_per_match` (or, equivalently, lowering `tournament_sliding_window` to free games for the comparisons that matter).
4. **EMA checkpoint for tournament eval.** *Was #3, demoted one slot.* The yes-no-no-no-yes-no-no-no oscillation around iter 6 *might* mean latest-checkpoint-as-eval-target is too noisy — but with the CI math above, half of those rejections may be false negatives, not real regressions. Ship after #3 so the EMA-vs-no-EMA comparison has signal to measure against. Independent and low-risk; just down-ranked.
5. **Deterministic eval mode.** *Promoted from #11, paired with #3.* No fixed-seed path in `_play_match`. Together with #3, this is the trustworthy-comparisons foundation that everything downstream depends on.
...
13. **Rework move encoder to a collision-free scheme.** Audit 2026-04-14 found `((src*31 + dst) % 5848)` for 85-position src/dst produces only 2687 distinct values across 7140 (src,dst) pairs — 62% collision rate, 3161 slots structurally unreachable (output bounded to [0, 2688]). Multiple distinct ring moves share policy indices in MCTS training targets; the network can't learn to distinguish them. Candidate clean encoding: direct `(src_idx, dst_idx)` pair indexing (85 × 84 = 7140 slots, no collisions, fits inside the current 7395 budget). Invasive — touches `move_to_index`, `index_to_move`, and any consumer that assumes current slot layout. Deferred until after Track A items #1-#5 land so we're not stacking encoder changes on top of statistics/augmentation fixes.
