"""
Standalone unit tests for database utility functions.
"""
...
def run_basic_functionality_test():
    """
    Run a basic functionality test to demonstrate the utility functions.
    """
    print("\n=== Basic Functionality Test ===")
    
    # Create temporary database
    temp_dir = tempfile.mkdtemp()
    test_db = os.path.join(temp_dir, "demo.db")
    
    try:
        print(f"1. Initializing database at {test_db}")
        ...
        exp_id = create_experiment(
            "demo_experiment", "abc123", "main", 
            config, environment, 
            tags=["demo", "test"]
        )
        print(f"   ✓ Created experiment with ID: {exp_id}")
        
        print("3. Adding metrics")
        add_metric_to_experiment(exp_id, "accuracy", 0.95, 1)
        add_metric_to_experiment(exp_id, "loss", 0.05, 1)
        print("   ✓ Added 2 metrics")
        
        print("4. Querying experiment")
        exp = get_experiment_by_id(exp_id)
        ...
        print("5. Getting database statistics")
        stats = get_database_stats()
        print(f"   ✓ Database has {stats['experiment_count']} experiments, {stats['metric_count']} metrics")
        
        print("6. Testing bulk operations")
        bulk_metrics = [
            {"name": "accuracy", "value": 0.97, "iteration": 2},
            {"name": "loss", "value": 0.03, "iteration": 2}
        ]
        add_metrics_bulk(exp_id, bulk_metrics)
        print("   ✓ Added bulk metrics")
        
        print("7. Testing querying")
        demo_exps = query_experiments(tags=["demo"])
        print(f"   ✓ Found {len(demo_exps)} experiments with 'demo' tag")
...
if __name__ == '__main__':
    ...
    print("Running database utility function tests...")
    
    # Run the basic functionality demo first
